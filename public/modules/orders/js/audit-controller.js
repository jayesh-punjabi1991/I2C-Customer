define(['angular', './module'], function(angular, controllers) {
    ...
    controllers.controller('AuditCtrl', ['$scope', '$log', '$timeout', 'OrdersService', '$http', 'PredixUserService', '$window', '$filter', '$state','$stateParams','$q', function($scope, $log, $timeout, OrdersService, $http, PredixUserService, $window, $filter, $state, $stateParams, $q) {
      ...
      var count = 1;
      OrdersService.getOrderAuditRecords($stateParams.id).success(function (response) {
        ...
        angular.forEach($scope.crList,  function (value, key) {
          value.cr_link = "<a href='/changeRequest/Accepted/" + value.change_req_id + "'>" + value.change_req_id + "</a>";
      ...
      $scope.getPayments = function (x) {
        $scope.PLoading = true;
        OrdersService.getInvoiceAuditRecords(x).success(function (response) {
          console.log(response);
